...
V2.5 将智能体升级为"本地意图路由 + LLM 角色规划 + 有界工具编排 + 确定性安全守门员"架构：13 个工具由统一注册表管理 schema、权限、安全级别、超时和审计；能力目录声明 10 个角色（安全守门、实时感知、风险分析、无障碍导航、时序记忆、视觉观察、偏好管理、沟通、播报策略、运行监控），各自声明职责、权限边界和首选工具。每轮对话先经本地意图路由，最多预取 2 个本地只读工具；规则没识别出意图时（`intent=general`）才让 LLM 参与**任务分解**——它只决定由哪些专家角色参与及顺序理由，工具仍由角色表推导，`intent` 与预取工具不放权；随后由主责与协作角色（最多 3 个）各执行一个只读工具产出结构化证据，沟通结论经确定性安全守门员复核，最后才进入有缓存、有累计调用上限的 LLM 工具循环。原有通行安全边界、远程令牌、双模型生产管线和离线兜底保持不变。
...
- **LLM 角色规划（规则未命中时启用）**：`intent=general` 才让模型做任务分解，只决定参与角色与顺序理由，工具由角色表推导。规划调用有独立短超时、**不计入熔断**、失败静默回落规则计划；**通行问题、规则路由关闭、模型已降级时一律不规划**。规划来源（规则/LLM）与理由在页面上可核对
...
agent:               # 智能体
  ...
  architecture:      # 编排式多能力 Agent
    ...
    enable_llm_planner: true    # 规则未命中时由 LLM 决定参与角色（只决定角色，不决定工具）
    llm_planner_timeout: 8.0    # 规划调用独立超时（秒），失败静默回落规则计划且不计入熔断
...
**V2.5.0 增量（2026-09，智能体可解释性与自主性）**

- 编排链路与安全改写对照上屏：每条回复展示意图与置信度、主责角色、执行路径、决策依据、6 阶段状态、角色结论与工具轨迹；安全复核卡高亮被追加的那句，整句替换时并列原始与最终回复
- 新增 `GET /api/agent/cockpit` 与侧边栏「智能体架构」面板：10 角色职责与权限边界、13 工具读写/安全级别/超时、最近调用审计（按需拉取，不进轮询）
- 新增 **LLM 角色规划器**（`core/agent_planner.py`）：规则未命中（`intent=general`）时由模型决定参与角色与顺序理由；**只决定角色、不决定工具**，`intent` 与预取工具不放权；通行问题、规则路由关闭、模型已降级时一律不启用；规划调用有独立超时且**不计入熔断**，失败静默回落规则计划
- 安全后处理新增可审计的动作类型（`unchanged` / `disclaimer_appended` / `replaced`）与追加文案；模型中途不可用时如实标注「本地兜底」，不把模板回答包装成模型作答
- 复合通行问题改为「先确定性拒绝、再用导航工具补非通行建议」，补充文案全部取自工具输出，避免自撰内容被安全层判为授权措辞
- 新增 `evaluate.py e2e-init / e2e`：用**生产管线**跑人工真值，输出当前部署配置的精确率/召回率/高危类别漏检/灯色，自动记录模型 SHA256 等口径；存在未人工校正帧时**拒绝写入报告**
- 回归测试 64 → **104 项**；补充可执行的视障用户测试流程（见 `docs/用户测试方案.md`）
- 补测 CV 核心模块（跟踪趋势与轨迹过期、红绿灯 HSV 识别、风险类别/距离/位置分档与红灯加权），回归测试 104 → **178 项**；IoU、画面方位分带与红绿灯类别常量收敛为唯一实现（`core/geometry.py`、`core/labels.py`）
- 检测推理异常不再静默：引擎记录连续失败与最后错误并透出到状态接口、健康工具与模型上下文，本地兜底回答同步改为"检测不可用，无法确认周围情况"，避免把推理失败说成"未检测到目标"
